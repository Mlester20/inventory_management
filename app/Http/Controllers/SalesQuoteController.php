...
class SalesQuoteController extends Controller
{
    ...
    protected function draftValidationRules(): array
    {
        return [
            ...
            'items.*.tax_classification' => 'nullable|in:' . implode(',', array_keys(SalesQuoteItem::TAX_CLASSIFICATIONS)),
        ];
    }
    ...
    {
        $salesQuote->load('customer', 'preparedBy', 'items.genericName', 'items.product', 'salesOrder');
        $users = User::orderBy('name')->get();

        $vatPreview = $this->buildVatPreview($salesQuote->items);

        return view('admin.sales-quotes.show', compact('salesQuote', 'users', 'vatPreview'));
    }
