...
<script>
    ...
    function renderAvailability(cells, items, index, salesOrderItemId, remainingQty, unit, preselect = null) {
        ...
        // A "not delivering this item yet" checkbox (rather than an option
        // buried inside the item dropdown — harder to notice, per Sir's
        // feedback) disables the select and Qty when checked, so a genuine
        // partial fulfillment (this Sales Order line's item hasn't arrived
        // at the Warehouse yet) submits a blank line the backend skips
        // instead of erroring.
        cells.item.innerHTML = `
        ...
        const skipCheckbox = cells.item.querySelector('.skip-item-checkbox');
        ...
        // Checking "Not delivering this item yet" disables the select and
        // Qty — a disabled field never posts, so this line arrives at the
        // backend with a blank product_batch_id/qty, which
        // DeliveryReceiptService::applyItems() skips rather than erroring.
        function syncSkipState() {
            const skipped = skipCheckbox.checked;
            itemSelect.disabled = skipped;
        ...
        skipCheckbox.addEventListener('change', syncSkipState);
